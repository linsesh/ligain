...
export function getTitleKey(baseScore: number | undefined, score: ScoreData | null): string {
...
function StaticPoints({ points }: { points: number }) {
  const { t } = useTranslation();
  const isPositive = points > 0;
  const bgColor = isPositive ? '#4a5d23' : colors.formLoss;
  const prefix = points > 0 ? '+ ' : points < 0 ? '- ' : '';
  const label = `${prefix}${Math.abs(points)} ${t('game.points')}`;
...
export function ScoreBreakdown({ score, animate = true }: ScoreBreakdownProps) {
  const { t } = useTranslation();
  ...
  return (
    <>
      ...
      {animate ? (
        ...
        <StaticPoints points={totalPoints} />
      )}
    </>
  );
}
